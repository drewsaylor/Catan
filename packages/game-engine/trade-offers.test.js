...
test("TRADE_OFFER_CREATE: validates recipient and resource shapes", () => {
  ...
  assert.deepEqual(
    applyAction(game, { type: "TRADE_OFFER_CREATE", to: "A", give: { wood: 1 }, want: { brick: 1 } }, "A"),
    {
      error: { code: "BAD_TRADE_TO" }
    }
  );
  assert.deepEqual(
    applyAction(game, { type: "TRADE_OFFER_CREATE", to: "Z", give: { wood: 1 }, want: { brick: 1 } }, "A"),
    {
      error: { code: "BAD_TRADE_TO" }
    }
  );
  assert.deepEqual(applyAction(game, { type: "TRADE_OFFER_CREATE", to: "B", give: {}, want: { brick: 1 } }, "A"), {
    error: { code: "BAD_TRADE" }
  });
  assert.deepEqual(applyAction(game, { type: "TRADE_OFFER_CREATE", to: "B", give: { wood: 1 }, want: {} }, "A"), {
    error: { code: "BAD_TRADE" }
  });
...
test("TRADE_OFFER_CANCEL: only owner can cancel an open offer", () => {
  ...
  const created = applyAction(
    game,
    { type: "TRADE_OFFER_CREATE", to: "B", give: { wood: 1 }, want: { brick: 1 } },
    "A"
  );
  assert.ok(created.game);
  const offerId = created.game.tradeOffers[0].id;

  assert.deepEqual(applyAction(created.game, { type: "TRADE_OFFER_CANCEL", offerId }, "B"), {
  ...
  assert.deepEqual(applyAction(cancelled.game, { type: "TRADE_OFFER_CANCEL", offerId }, "A"), {
    error: { code: "OFFER_CLOSED" }
  });
  assert.deepEqual(applyAction(cancelled.game, { type: "TRADE_OFFER_CANCEL", offerId: "nope" }, "A"), {
    error: { code: "NO_SUCH_OFFER" }
  });
});
...
test("TRADE_OFFER_RESPOND: enforces targeting and closes offers correctly", () => {
  ...
  assert.deepEqual(applyAction(created.game, { type: "TRADE_OFFER_RESPOND", offerId, response: "accept" }, "C"), {
    error: { code: "NOT_FOR_YOU" }
  });
  ...
  assert.deepEqual(applyAction(rejected.game, { type: "TRADE_OFFER_RESPOND", offerId, response: "accept" }, "B"), {
    error: { code: "OFFER_CLOSED" }
  });
});
...
test("TRADE_OFFER_RESPOND: global offers close only after everyone rejects", () => {
  ...
  const created = applyAction(
    game,
    { type: "TRADE_OFFER_CREATE", to: "all", give: { wood: 1 }, want: { brick: 1 } },
    "A"
  );
  assert.ok(created.game);
  const offerId = created.game.tradeOffers[0].id;
  ...
  assert.deepEqual(applyAction(bReject.game, { type: "TRADE_OFFER_RESPOND", offerId, response: "reject" }, "B"), {
...
test("END_TURN: expires only the current player's open offers", () => {
  ...
  const accepted = applyAction(
    created.game,
    { type: "TRADE_OFFER_RESPOND", offerId: created.game.tradeOffers[0].id, response: "accept" },
    "B"
  );
  assert.ok(accepted.game);
  assert.equal(accepted.game.tradeOffers[0].status, "accepted");

  const created2 = applyAction(
    accepted.game,
    { type: "TRADE_OFFER_CREATE", to: "all", give: { sheep: 1 }, want: { ore: 1 } },
    "A"
  );
